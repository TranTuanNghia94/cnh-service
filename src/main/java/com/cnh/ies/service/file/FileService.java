...
public class FileService {
    ...
    public static final String EXPORT_S3_PREFIX = "exports";
    public static final String EXPORT_CATEGORY = "exports";
    ...
    /**
     * Uploads generated bytes (e.g. Excel export) to S3 and persists {@code file_infos}.
     */
    @Loggable(slowThresholdMs = 5_000)
    public PaymentFileUploadInfo uploadGeneratedFile(
            byte[] content,
        ...
        if (content == null || content.length == 0) {
            throw new ApiException(ApiException.ErrorCode.BAD_REQUEST, "File content is required",
                    HttpStatus.BAD_REQUEST.value(), requestId);
        }
        String safeName = originalFileName == null || originalFileName.isBlank() ? "export.xlsx" : originalFileName.trim();
        String prefix = s3PathPrefix == null || s3PathPrefix.isBlank() ? EXPORT_S3_PREFIX : s3PathPrefix.trim();
        String safeCategory = category == null || category.isBlank() ? EXPORT_CATEGORY : category.trim().toLowerCase(Locale.ROOT);
        String key = buildS3Key(prefix, safeCategory, safeName);

        log.info("Uploading generated file '{}' ({} bytes) to s3://{}/{}", safeName, content.length, bucketName, key);
        ...
        try {
            ...
            info.setContentType(XLSX_CONTENT_TYPE);
            info.setSize((long) content.length);
            info.setCategory(safeCategory);
            info.setFilePath(key);
            info.setFileUrl(buildS3Url(key));
            info.setAttachmentType(PaymentFileAttachmentType.PAPER.name());
            PaymentFileUploadInfo saved = saveFileInfo(info, requestId);
            log.info("Generated file uploaded: id={} path={}", saved.getId(), saved.getFilePath());
            return saved;
        } catch (S3Exception e) {
            if (e.statusCode() == 403) {
                throw new ApiException(ApiException.ErrorCode.FORBIDDEN,
                        "S3 upload denied: " + e.awsErrorDetails().errorMessage(),
                        HttpStatus.FORBIDDEN.value(), requestId);
            }
            log.error("S3 upload failed for key '{}': {}", key, e.getMessage(), e);
            throw new ApiException(ApiException.ErrorCode.INTERNAL_ERROR, "Upload to S3 failed: " + e.getMessage(),
            ...
            log.error("Unexpected error uploading generated file to S3", e);
            throw new ApiException(ApiException.ErrorCode.INTERNAL_ERROR, "Upload to S3 failed: " + e.getMessage(),
                    HttpStatus.INTERNAL_SERVER_ERROR.value(), requestId);
        }
    }
    ...
    /**
     * Generates a pre-signed GET URL for the given S3 key, valid for 24 h.
     */
    public String presignGetUrlForKey(String key) {
