...
public class VendorBanksService {
    ...
    public VendorBanksInfo createVendorBank(CreateVendorBanksRequest request, String requestId, VendorsEntity vendor) {
        try {
            ...
            log.info("Vendor bank created successfully | RequestId: {} request: {} vendor: {}", requestId, request,
                    vendor);
    ...
    public List<VendorBanksInfo> createVendorBanks(List<CreateVendorBanksRequest> request, String requestId,
            VendorsEntity vendor) {
        try {
            log.info("Creating vendor banks | RequestId: {} request: {} vendor: {}", requestId, request, vendor);

            List<VendorBanksEntity> vendorBanks = request.stream()
                    .map(bank -> vendorBanksMapper.toVendorBanksEntity(bank, vendor)).collect(Collectors.toList());
            vendorBanksRepo.saveAll(vendorBanks);

            log.info("Vendor banks created successfully | RequestId: {} request: {} vendor: {}", requestId, request,
                    vendor);
    ...
    public List<VendorBanksInfo> updateVendorBanks(List<UpdateVendorBanksRequest> request, String requestId,
            VendorsEntity vendor) {
        try {
            log.info("Updating vendor banks | RequestId: {} request: {} vendor: {}", requestId, request, vendor);

            List<VendorBanksEntity> vendorBanks = request.stream()
                    .map(bank -> vendorBanksMapper.toVendorBanksEntity(bank, vendor)).collect(Collectors.toList());
            vendorBanksRepo.saveAll(vendorBanks);

            log.info("Vendor banks updated successfully | RequestId: {} request: {} vendor: {}", requestId, request,
                    vendor);

            return vendorBanksMapper.toVendorBanksInfoList(vendorBanks);
        } catch (Exception e) {
            log.error("Error updating vendor banks", e);
            throw new ApiException(ApiException.ErrorCode.INTERNAL_ERROR, e.getMessage(),
                    HttpStatus.INTERNAL_SERVER_ERROR.value(), requestId);
        }
    }
    ...
    public String deleteVendorBank(String requestId, String vendorBankId) {
        try {
            ...
            if (vendorBank.isEmpty()) {
                ...
                throw new ApiException(ApiException.ErrorCode.NOT_FOUND, "Vendor bank not found",
